...
`;
		...
		function SkillHubSection({ openPath, api, t }) {
			(0, react.useEffect)(() => {
				if (document.getElementById("dsh-skill-style") === null) {
					const style = document.createElement("style");
					style.id = "dsh-skill-style";
		...
		const DESCRIPTORS = [
			...
			},
			{
				...
				parameters: [{
					...
					wire: "dirPath",
					source: "json",
					codec: passCodec("dsh-skill#DirPath")
				}],
			...
			},
			{
				...
				parameters: [{
					...
					wire: "command",
					source: "json",
					codec: passCodec("dsh-skill#HubCommand")
				}],
		...
		*/
		async function apply(ctx) {
			...
			ctx.effect(() => () => {
				disposeRemote();
			}, "dsh-skill: remote descriptor mount");
			let calls;
